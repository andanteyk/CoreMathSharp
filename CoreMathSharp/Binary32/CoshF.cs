...
public static partial class StrictMathF
{
    ...
    public static float Cosh(float x)
    {
        ...
        ReadOnlySpan<double> ch = [1.0, 0.02166084939249829, 0.0002345961982022468, 1.6938509724129055e-06, 9.1725627017026289e-09, 3.973729405780548e-11, 1.4345723178374038e-13];
        ...
        if (ax > 0x8565a9f8u)
        {
            ...
            float r1 = 2.0f * 3.4028234663852886e+38f;
            return r1;
        }
        ...
        if (ax < 0x7c000000u)
        {
            if (ax < 0x74000000u)
            {
                if (ax < 0x66000000u)
                {
                    return FusedMultiplyAdd(Abs(x), 2.9802322387695312e-08f, 1.0f);
                }

                return (0.5f * x) * x + 1.0f;
            }

            ReadOnlySpan<double> cp = [0.49999999999999839, 0.041666666667488188, 0.0013888887416776143, 2.4812354013894482e-05];
            double z2 = z * z, z4 = z2 * z2;
            return (float)(1.0 + z2 * ((cp[0] + z2 * cp[1]) + z4 * (cp[2] + z2 * cp[3])));
        }
        ...
        ulong sp = tb[(int)jp & 31] + ((ulong)(jp >> 5) << 52), sm = tb[(int)jm & 31] + ((ulong)(jm >> 5) << 52);

        double te = c[0] + h2 * c[2], to = c[1] + h2 * c[3];
        double rp = Polyfill.UInt64BitsToDouble(sp) * (te + h * to), rm = Polyfill.UInt64BitsToDouble(sm) * (te - h * to), r = rp + rm;

        float ub = (float)r, lb = (float)(r - 1.45e-10 * r);
        if (ub != lb)
        {
