...
public static partial class StrictMath
{
    ...
    public static double Asin(double x)
    {
        ...
        static double asinAcc(double x)
        {
        ...
        if (e >= 0)
        {
            ...
            if (e == 0 && m == 0)
            {
                return CopySign(1.5707963267948966, x) + CopySign(6.123233995736766e-17, x);
            }
            if (e == 0x400 && m != 0)
            {
                return x + x;
        ...
        else if (e < -6)
        {
            if (e < -26)
            {
                return FusedMultiplyAdd(x, 5.5511151231257827e-17, x);
            }
            ...
            ulo += 12ul << ss;      // e < -6; ss < 51; no overflow
            if (ulo < fiLo)
            {
                uhi++;
            }
        ...
        else
        {
            ...
            (ulo, uhi) = add128(ulo, uhi, 386ul << 55, 0);
            (dlo, dhi) = sub128(dlo, dhi, 386ul << 55, 0);
